...
void preset_angle(hcan_t* hcan, uint8_t id_num, float angle, float t, float param, int mode)
{
    ...
    if (mode == 0)
    {
        ...
        send_command(hcan, id_num,0x0C,data_list.byte_data,0);
    }
    else if(mode == 1)
    {
        float f_angle = angle;
...
void set_angle(hcan_t* hcan, uint8_t id_num, float angle, float speed, float param, int mode)
{
    ...
    if (mode == 0)
    {
        ...
        send_command(hcan, id_num,0x19,data_list.byte_data,0);
    }
    else if (mode == 1)
    {
        if ((speed > 0)&&(param > 0))
        {
            ...
            send_command(hcan, id_num,0x1A,data_list.byte_data,0);
        }
    }
    else if (mode == 2)
    {
        ...
        send_command(hcan, id_num, 0x1B, data_list.byte_data, 0);
    }
    reply_state(id_num);
}
...
void set_angles(hcan_t* hcan, uint8_t *id_list, float *angle_list, float speed, float param, int mode, size_t n)
{
    ...
    if (last_count != n || state)
    {
        ...
        for (size_t i = 0; i < n; i++)
            current_angle_list[i] = get_state(hcan, id_list[i]).angle;
    }
    if (mode == 0)
    {
        for (size_t i = 0; i < n; i++)
            preset_angle(hcan, id_list[i],angle_list[i],speed,param,mode);
        ...
        send_command(hcan, 0,0x08,data_list.byte_data,0);  // 需要用标准帧（数据帧）进行发送，不能用远程帧
    }
    else if (mode == 1)
    {
        ...
        for (size_t i = 0; i < n; i++)
            preset_angle(hcan, id_list[i],angle_list[i],t,param,mode);
...
{
    step_angles(hcan, &id_num,&angle,speed,param,mode,1);
}
/**
 * @brief 多个电机相对角度控制函数。
...
void set_speed(hcan_t* hcan, uint8_t id_num, float speed, float param, int mode)
{
    ...
    send_command(hcan, id_num,0x1c,data_list.byte_data,0);
    reply_state(id_num);
}
/**
 * @brief 多个电机速度控制函数。
...
void set_speeds(hcan_t* hcan, uint8_t *id_list, float *speed_list, float param, float mode, size_t n)
{
    ...
    for (size_t i = 0; i < n; i++)
        preset_speed(hcan, id_list[i], speed_list[i], param, mode);
...
void set_torque(hcan_t* hcan, uint8_t id_num, float torque, float param, int mode)
{
    ...
    int type_data[3]= {0,2,1};
    format_data(value_data,type_data,3,"encode");
    send_command(hcan, id_num,0x1d,data_list.byte_data,0);
    reply_state(id_num);
}
/**
 * @brief 多个个电机力矩控制函数。
...
void set_torques(hcan_t* hcan, uint8_t *id_list, float *torque_list, float param, int mode, size_t n)
{
    ...
    for (size_t i = 0; i < n; i++)
        preset_torque(hcan, id_list[i], torque_list[i], param, mode);
...
{
    float factor = 0.01;
    preset_angle(hcan, id_num,pos,vel, tff, 2);
...
void estop(hcan_t* hcan, uint8_t id_num)
{
    ...
    int type_data[3]= {3,1,1};
    format_data(value_data,type_data,3,"encode");
    send_command(hcan, id_num,0x08,data_list.byte_data,0);  // 需要用标准帧（数据帧）进行发送，不能用远程帧
}
// 参数设置功能
...
{
    write_property(hcan, id_num,31001,3,new_id);
    save_config(hcan, new_id);
}
/**
 * @brief 设置电机串口波特率。
 * 设置UART串口波特率（掉电保存）
...
{
    write_property(hcan, id_num,10001,3,baud_rate);
    save_config(hcan, id_num);
}
/**
 * @brief 设置电机CAN波特率。
 * 设置CAN波特率（掉电保存）
...
{
    write_property(hcan, id_num,21001,3,baud_rate);
    save_config(hcan, id_num);
}
/* @brief 设置电机模式。
* 设置电机进入不同的控制模式。
...
{
    if (mode == 1)
        write_property(hcan, id_num,30003,3,AXIS_STATE_IDLE);
    else if (mode == 2)
        write_property(hcan, id_num,30003,3, AXIS_STATE_CLOSED_LOOP_CONTROL);
}
/**
 * @brief 设置电机零点位置函数
...
void set_GPIO_mode(hcan_t* hcan, uint8_t id_num, uint8_t mode, uint32_t param)
{
    ...
    get_GPIO_mode(hcan, id_num, &enable_uart, &enable_step_dir, NULL);
    if (enable_uart != !mode)
    {
        write_property(hcan, id_num, 10008, 1, !mode);
        SERVO_DELAY(sleep_time);
    }
    if (enable_step_dir != mode)
    {
        write_property(hcan, id_num, 31006, 1, mode);
        SERVO_DELAY(sleep_time);
    }
    if (mode == 0) write_property(hcan, id_num, 10001, 3, param);
    else if (mode == 1) write_property(hcan, id_num, 38019, 0, param);
    SERVO_DELAY(sleep_time);
    save_config(hcan, id_num);
    SERVO_DELAY(sleep_time);
    reboot(hcan, id_num);
...
int8_t set_angle_range(hcan_t* hcan, uint8_t id_num, float angle_min, float angle_max)
{
    ...
    float pos_vel = get_state(hcan, id_num).angle;
    if (READ_FLAG == 1)
    {
        if (pos_vel < angle_min || pos_vel > angle_max || READ_FLAG != 1) return -1;
        write_property(hcan, id_num, 38010, 0, angle_min);
        write_property(hcan, id_num, 38011, 0, angle_max);
        write_property(hcan, id_num, 31202, 1, 1);
        save_config(hcan, id_num);
        return 1;
    }
    else
...
{
    write_property(hcan, id_num,35104,3, mode);
    TRAJ_MODE = mode;
}
/**
 * @brief 修改电机属性参数
...
void write_property(hcan_t* hcan, uint8_t id_num,unsigned short param_address,int8_t param_type,float value)
{
    ...
    format_data(value_data,type_data,3,"encode");
    send_command(hcan, id_num, 0x1F,data_list.byte_data,0);  //需要用标准帧（数据帧）进行发送，不能用远程帧
}
/**
 * @brief 读取电机ID。
 *
...
{
    return read_property(hcan, id_num,31001,3);
}
/**
 * @brief 读取电机的当前位置和速度
...
struct servo_state get_state(hcan_t* hcan, uint8_t id_num)
{
    ...
    format_data(value_data,type_data,3,"encode");
    send_command(hcan, id_num,0x1E,data_list.byte_data,0);// 需要用标准帧（数据帧）进行发送，不能用远程帧
		READ_FLAG=0;
    receive_data();
...
struct servo_volcur get_volcur(hcan_t* hcan, uint8_t id_num)
{
    ...
    volcur.vol  = read_property(hcan, id_num, 1, 0);
    if(READ_FLAG==1) {
        volcur.cur  = read_property(hcan, id_num, 33206,0);
    }
    else READ_FLAG=-1;
    return volcur;
}
...
int8_t get_GPIO_mode(hcan_t* hcan, uint8_t id_num, uint8_t *enable_uart, uint8_t *enable_step_dir, uint32_t *n)
{
    ...
    if (enable_uart != NULL) *enable_uart = read_property(hcan, id_num,10008,1) == 0 ? 0 : 1;
    if (enable_step_dir != NULL) *enable_step_dir = read_property(hcan, id_num,31006,1) == 0 ? 0 : 1;
    if (READ_FLAG == 1)
    {
        if (*enable_uart && !*enable_step_dir)
        {
            if (n != NULL)
                *n = read_property(hcan, id_num,10001,3);
            return 1;
        }
        else if (!*enable_uart && *enable_step_dir)
        {
            if (n != NULL)
                *n = read_property(hcan, id_num,38019,0);
            return 1;
        }
        return -1;
    }
...
int8_t dump_error(hcan_t* hcan, uint8_t id_num)
{
    ...
    unsigned int axis_error = read_property(hcan, id_num, 30001,3);
    if(READ_FLAG==1&&axis_error!=0)
        return 1;
    else if(READ_FLAG!=1)
        return -1;
    unsigned int motor_error = read_property(hcan, id_num, 33001,3);
    if(READ_FLAG==1&&motor_error!=0)
        return 2;
    else if(READ_FLAG!=1)
        return -2;
    unsigned int controller_error = read_property(hcan, id_num, 32001,3);
    if(READ_FLAG==1&&controller_error!=0)
        return 3;
    else if(READ_FLAG!=1)
        return -3;
    unsigned int encoder_error = read_property(hcan, id_num, 34001,3);
    if(READ_FLAG==1&&encoder_error!=0)
        return 4;
    else if(READ_FLAG!=1)
        return -4;
    unsigned int can_error = read_property(hcan, id_num, 20001,3);
    if(READ_FLAG==1&&can_error!=0)
        return 5;
    else if(READ_FLAG!=1)
        return -5;
    unsigned int fet_thermistor_error = read_property(hcan, id_num, 36001,3);
    if(READ_FLAG==1&&fet_thermistor_error!=0)
        return 6;
    else if(READ_FLAG!=1)
        return -6;
    unsigned int motor_thermistor_error = read_property(hcan, id_num, 37001,3);
...
void position_done(hcan_t* hcan, uint8_t id_num)
{
    ...
    {
        traj_done = read_property(hcan, id_num, 32008,3);
    }
}
/**
 * @brief 多个电机等待函数
